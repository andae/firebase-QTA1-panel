 ...
 // Bar chart
  ...
  // Pie chart
    ...
    constructor(private colors: ColorsService, private http: Http, public af :AngularFireDatabase, @Inject(LOCALE_ID) private _locale: string) {
     ...
     af.list('/menusItems').valueChanges().subscribe((res)=>{
         this.menuItems = res.length;
     });
      af.list('/catlelang').valueChanges().subscribe((res)=>{
         this.categories = res.length;
     });

      this.ordersDataRef = af.list('/orders/', ref => ref.orderByChild('jenislapak').equalTo('lelang'));
    ...
    ngOnInit() {
     let lastOrder:any[]=[];     
      ...
      this.orderObservable = this.af.list('/orders/', ref => ref.orderByChild('jenislapak').equalTo('lelang').limitToFirst(1)).valueChanges();


      // this.usersDataRef = af.list('/users', ref =>  ref.orderByChild('role').equalTo('Mitra'));
      ...
      this.orderObservable.subscribe((res)=>{             
            ...
            //Pie Chart
            this.af.list('/catlelang').snapshotChanges().map(changes => {
                return changes.map(c => ({ key: c.payload.key, ...c.payload.val() }));
            }).subscribe((category)=>{
                  this.af.list('/menusItems').valueChanges().subscribe((menuitem:any)=>{
