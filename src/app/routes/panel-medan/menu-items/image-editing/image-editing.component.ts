...
export class ImageEditingComponent implements OnInit {
  ...
  constructor(public activatedRoute: ActivatedRoute, public af: AngularFireDatabase,
  	private route: ActivatedRoute, public router: Router, private uploadService: UploadService) { 
  ...
  ngOnInit() {
  	this.activatedRoute.params.map(params => params['id']).subscribe((menukey) => {
  		if(menukey != null) {
  			this.itemsDocData = this.af.object('menuItems/' + menukey);
  ...
  simpanGambar() {
      ...
      if(this.urlGambarBesar != undefined && this.urlGambarKecil != undefined) {
      	  this.uploadService.deleteUploadSingle(this.namaGambarLastBesar);
          this.uploadService.deleteUploadSingle(this.namaGambarLastKecil);
