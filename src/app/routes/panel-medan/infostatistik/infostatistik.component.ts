...
@Component({
  selector: 'app-infostatistik',
  templateUrl: './infostatistik.component.html',
  styleUrls: ['./infostatistik.component.scss'],
  providers: [ PushNotificationService ]
})
export class InfostatistikComponent implements OnInit {
  ...
  getNewsDetailList() {
    this.newsColl = this.afs.list('InfoStatistiks', ref => {
  ...
  getSearchItems(ev:any){
    let val = ev;     
      this.newsObservable = this.afs.list('/InfoStatistiks',ref => ref.orderByChild('title').startAt(val.charAt(0).toUpperCase() + val.slice(1))
  ...
  viewArtikels(key) {
    this.router.navigate(['/infostatistis/viewInfostatistiks', key])
  }

  updateArtikels(key) {
    this.router.navigate(['/infostatistis/updateInfostatistiks', key]);
  }
  ...
  deleteArtikels(key,i) {
    swal({
      ...
      this.newsDoc = this.afs.object('InfoStatistiks/' + key);
          ...
          if(count ==1) {
            ...
            let dr = firebase.storage().refFromURL(res.thumb);
              ...
              dr.delete().then(sucess => {
                 this.newsDoc.remove();
                 swal('Dihapus!', 'Informasi dan Statistik telah dihapus.', 'success');
              }).catch(error => {
              });
            }
        });
      } else {
        swal('Dibatalkan', 'Informasi dan Statistik masih ada :)', 'error');
      }
    });
  }
}
