...
export class ViewOrderlComponent{
  p:any;
  userDetails:any={};
  address:any;
  orderDetails:any={};
  orderldataRef: AngularFireObject<any>;
  ...
  constructor(private route: ActivatedRoute,  public router: Router, public af: AngularFireDatabase,public toastr: ToastrService) {
  	 	 	this.route.params.map(params => params['id']).subscribe((Id) => {
  	 	        if(Id != null) {
      		        this.orderldataRef = this.af.object('/orders/' + Id);
                  this.orderId = Id;
                  this.orderObservable = this.orderldataRef.valueChanges();
  ...
  generateInvoice(){
    this.router.navigate(['/orderl/invoice',this.orderId])
  }
