...
@Component({
  selector: 'app-update-artikels',
  templateUrl: './update-artikels.component.html',
  styleUrls: ['./update-artikels.component.scss']
})
export class UpdateArtikelsComponent implements OnInit {

  public dataUploading: number;
  news:any = { };

  gambarArtikel: any = this.news.thumb;
  namaArtikel;
  ...
  ngOnInit() {
    this.activatedRoute.params.map(params => params['id']).subscribe((Id) => {
      if(Id != null) {
        this.newsDocData = this.afs.object('Artikels/'+Id);
           this.newsObjectObservable = this.newsDocData.valueChanges();

           this.newsObjectObservable.subscribe(res => {
             if(res != null) {
               this.news = res;
               this.gambarArtikel = res.thumb;
               this.namaArtikel = res.namaGambar;
             }
           })

           this.CatCollList = this.afs.list('kategoriartikel');
  ...
  updateNews(form: NgForm) {
    ...
    if(this.urlGambar == undefined) {
      this.newsDocData.update({
       ...
       this.deleteUploadSingle(this.namaArtikel);
       ...
       that.newsDocData.update({
              ...
              this.toastr.warning('Artikel tidak diupdate !', 'Warning!');
            })
     }
  }
  ...
  nextTo() {
    ...
    this.toastr.info('Artikel berhasil diuodate!', 'Success!');
    this.router.navigate(['/artikels/manageArtikels']);
  }
