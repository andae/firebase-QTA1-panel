  ...
  // Pie chart
    ...
    constructor(private colors: ColorsService, private http: Http, public af :AngularFireDatabase, @Inject(LOCALE_ID) private _locale: string) {
     ...
     af.list('/menuItems').valueChanges().subscribe((res)=>{
      ...
      this.ordersDataRef = af.list('/orders/', ref => ref.orderByChild('jenislapak').equalTo('toko'));
    ...
    ngOnInit() {
     let lastOrder:any[]=[];     
      ...
      this.orderObservable = this.af.list('/orders/', ref => ref.orderByChild('jenislapak').equalTo('toko').limitToFirst(1)).valueChanges();
